...
public class Traverse {
  ...
  public static boolean reachable(Map<Integer, Set<Integer>> graph, int start, int end, Set<Integer> visited)
  {
    ...
    if(visited.contains(start) || graph == null || !graph.containsKey(start) || graph.get(start) == null)
    {
      return false;
    }
    ...
    {
      if(reachable(graph, n, end, visited))
      {
        return true;
      }
    }
  ...
  public static void printGossipers(Person person, Set<Person> visited)
  {
    ...
    for(Person c : person.getConfidants())
    {
      printGossipers(c, visited);
    }
  }
}
